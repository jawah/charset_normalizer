...
class ProbeChaos:
    ...
    def __init__(self, string, giveup_threshold=0.09, bonus_bom_sig=False, bonus_multi_byte=False):
        ...
        self.undefined_range_count = 0
        self.encountered_private_letter_use = 0
    ...
    def _probe(self):
        ...
        str_len = len(self._string)

        for c, i_ in zip(self._string, range(0, str_len)):

            if not c__:
                state_ = (i_ / str_len >= 0.4)
            ...
            if u_name is not None:
                ...
                if UnicodeRangeIdentify.is_private_use_only(c) is True:
                    self.encountered_private_letter_use += 1
